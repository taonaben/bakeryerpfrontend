...
import type { FormulaLineType, FormulaStatus } from '../types/models';
...
/** Map a persisted FormulaLine to the editor's local shape */
const toEntryLine = (line: {
  id: string;
  sequence: number;
  ...
  localId: line.id,           // use the real id as localId so we can send it back
  sequence: line.sequence,
  line_type: line.line_type,
  product: line.product ?? '',
  quantity: line.quantity !== undefined ? String(line.quantity) : '',
  text: line.text ?? '',
  location: '',               // not stored server-side; UI-only
});
...
const EditFormulaPage: React.FC = () => {
  ...
  // ─── Load formula + products in parallel ─────
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const [formula, productList] = await Promise.all([
          formulationService.fetchFormula(id),
        ...
        setLines(
          formula.lines.length > 0
            ? formula.lines.map(toEntryLine)
            : [createLine(1), createLine(2), createLine(3)],
        );
      } catch (err: any) {
      ...
      prev.map((line) => {
        ...
        if (key === 'product') {
          ...
          return {
            ...
            text: selectedProduct?.name ?? line.text,
          };
        }
  ...
  const handleDragStart = (lineId: string) => setDraggingLineId(lineId);
  ...
  const handleDrop = () => setDraggingLineId(null);
  const handleDragEnd = () => setDraggingLineId(null);
  ...
  if (loading) {
    return (
      <div className="formula-entry-page">
        ...
        <div style={{ padding: '2rem', color: 'var(--text-secondary)' }}>Loading formula…</div>
      </div>
    );
  }
...
// ──────────────────────────────────────────────
// UUID check — persisted lines have real UUIDs;
// new lines get a generated localId string.
// ──────────────────────────────────────────────
